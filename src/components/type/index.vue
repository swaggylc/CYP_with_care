  ...
  <el-card>
    <el-form inline>
      <el-form-item label="一级分类">
        <el-select
          v-model="typeStore.OneId"
          @change="changeOne"
          :disabled="scene != 0"
        >
          <el-option
            v-for="item in typeStore.ArrTypeOne"
            :key="item.id"
            :label="item.name"
            :value="item.id"
          ></el-option>
      ...
      <el-form-item label="二级分类">
        <el-select
          v-model="typeStore.TwoId"
          @change="changeTwo"
          :disabled="scene != 0"
        >
          <el-option
            v-for="item in typeStore.ArrTypeTwo"
            :key="item.id"
            :label="item.name"
            :value="item.id"
          ></el-option>
      ...
      <el-form-item label="三级分类">
        <el-select v-model="typeStore.ThreeId" :disabled="scene != 0">
          <el-option
            v-for="item in typeStore.ArrTypeThree"
            :key="item.id"
            :label="item.name"
            :value="item.id"
          ></el-option>
